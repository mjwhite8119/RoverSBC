...
class RoverTF2Publisher : public rclcpp::Node
{
public:
    RoverTF2Publisher()
        : Node("rover_tf2_broadcaster_node")
    {
        tf_broadcaster_ =
            std::make_unique<tf2_ros::TransformBroadcaster>(*this);
        subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/odom", 10,
            std::bind(&RoverTF2Publisher::odom_callback, this, _1));
    }
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::spin(std::make_shared<RoverTF2Publisher>());
    rclcpp::shutdown();
    return 0;
}